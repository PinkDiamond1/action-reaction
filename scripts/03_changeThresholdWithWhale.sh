...
alltkns=0
findTokens=$(jq -r --argjson alltkns 0 --arg policy_id "$policy_id" --arg token_name "$token_name" 'to_entries[] | select(.value.value[$policy_id][$token_name] >= 1) | .value.value[$policy_id][$token_name] | . + $alltkns' tmp/buyer_utxo.json)
total=0
...
UTXO_VALUE=$(${cli} transaction calculate-min-required-utxo \
    ...
    --tx-out="${buyer_address} + 5000000 + ${action_asset}" | tr -dc '0-9')
#
...
buyer_address_out="${buyer_address} + ${UTXO_VALUE} + ${action_asset}"
...
TXIN=$(jq -r --arg alltxin "" --arg policy_id "$policy_id" --arg token_name "$token_name" 'to_entries[] | select((.value.value | length < 2) or .value.value[$policy_id][$token_name] > 1) | .key | . + $alltxin + " --tx-in"' tmp/buyer_utxo.json)
buyer_tx_in=${TXIN::-8}
