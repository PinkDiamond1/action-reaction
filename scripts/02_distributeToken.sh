...
buyer_address=$(cat wallets/delegator-wallet/payment.addr)
# buyer_address=""
#
...
token_number=$(cat data/datums/distro_datum.json | jq -r .fields[1].int)

amount=$(python3 -c "from reward import amount;print(amount(${token_number}))")
name=${token_name}
#
MINT_ASSET="${amount} ${policy_id}.${name}"
...
script_ref_utxo=$(${cli} transaction txid --tx-file tmp/ref-distro-utxo.signed)

# update the datum into the new state
t=$(cat data/datums/distro_datum.json | jq .fields[1].int)
ft=$((t+1))
variable=${ft}; jq -r --argjson variable $variable '.fields[1].int=$variable' data/datums/distro_datum.json > data/datums/distro_datum-new.json
mv data/datums/distro_datum-new.json data/datums/distro_datum.json
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file data/redeemers/mint_redeemer.json \
    ...
    --policy-id="${policy_id}" \
    --mint-reference-tx-in-redeemer-file data/redeemers/mint_redeemer.json \
    --testnet-magic ${testnet_magic})
