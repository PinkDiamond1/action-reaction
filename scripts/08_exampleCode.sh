...
#
script_path="../example-contract/example-contract.plutus"
...
TXIN=$(jq -r --arg alltxin "" --arg policy_id "$policy_id" --arg token_name "$token_name" 'to_entries[] | select((.value.value | length < 2)) | .key | . + $alltxin + " --tx-in"' tmp/seller_utxo.json)
seller_tx_in=${TXIN::-8}
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file data/redeemers/unlock_redeemer.json \
    --tx-out="${script_address_out}" \
