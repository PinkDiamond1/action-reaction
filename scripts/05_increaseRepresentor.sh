...
#
variable=${delegator_pkh}; jq --arg variable "$variable" '.fields[0].fields[1].bytes=$variable' data/redeemers/increase_redeemer.json > data/redeemers/increase_redeemer-new.json
mv data/redeemers/increase_redeemer-new.json data/redeemers/increase_redeemer.json
...
alltkns=0
findTokens=$(jq -r --argjson alltkns 0 --arg policy_id "$policy_id" --arg token_name "$token_name" 'to_entries[] | select(.value.value[$policy_id][$token_name] >= 1) | .value.value[$policy_id][$token_name] | . + $alltkns' tmp/delegator_utxo.json)
total=0
...
variable=${actionAmount}; jq -r --argjson variable "$variable" '.fields[0].fields[0].int=$variable' data/redeemers/increase_redeemer.json > data/redeemers/increase_redeemer-new.json
mv data/redeemers/increase_redeemer-new.json data/redeemers/increase_redeemer.json
...
script_address_out="${script_address} + ${starter_nft_min_utxo} + ${action_asset}"
delegator_address_out="${delegator_address} + ${starter_nft_min_utxo} + ${mint_asset}"
...
TXIN=$(jq -r --arg alltxin "" --arg policy_id "$policy_id" --arg token_name "$token_name" 'to_entries[] | select((.value.value | length < 2) or .value.value[$policy_id][$token_name] >= 1) | .key | . + $alltxin + " --tx-in"' tmp/delegator_utxo.json)
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file data/redeemers/increase_redeemer.json \
    ...
    --policy-id="${iou_id}" \
    --mint-reference-tx-in-redeemer-file data/redeemers/increase_redeemer.json \
    --testnet-magic ${testnet_magic})
